...
| Market quotes and freshness | M2 - Implemented provider-independent groundwork, offline Schwab adapter, and interim one-app quote-evidence bridge | Not accepted | OneBot/VPS is only the temporary single Schwab-token owner; the target OneJournal-owned connector plane must support Schwab, IBKR, Moomoo, and later providers behind common contracts. Approved provider capture, private transfer, entitlement verification, a sanitized official fixture, connector security/ownership, single-owner cutover, durable ingestion, and end-to-end validation remain open | PNL-02 |
