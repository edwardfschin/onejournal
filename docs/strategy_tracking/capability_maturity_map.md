...
| Market quotes and freshness | M3 - Validated for the bounded Schwab T14 evidence and provider-independent local contracts | Not operationally accepted | ADR-0011 and its acceptance matrix approve same-connected-broker schedule authority, and the common resolver/freshness/importer boundary enforces exact provider/connection/quote/instrument/source lineage. Same-date capture `PNL-02-T14-SCHWAB-20260829-06` proves official real-time stock/option shapes, normal/closed-sentinel/shortened schedules, combined v2 authority, and closed-session valuation behavior. `schwab-quote-json-v2` preserves the observed equity `Closed` state as frozen and refuses valuation without exact authority or while the effective market remains open. OneBot/VPS is only the temporary token owner; target connector activation, approved runtime ingestion/migration, single-owner cutover, and end-to-end owner acceptance remain open | PNL-02 |
