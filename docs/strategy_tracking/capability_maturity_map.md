...
Snapshot date: 2026-09-06. This summary is derived from the current product
...
| Current positions and broker-current valuation | M3 - Validated for the bounded private single-owner Schwab scope | Not operationally accepted; bounded financial acceptance recorded 2026-09-05 for one exact 58-position result and fingerprint | PNL-03 provides 58/58/58 cost-basis, market-value, and unrealized-P&L availability with per-position Schwab reconciliation. Actual journal migration, authenticated API registration, continuous acquisition, production release, and complete transferred-lot realized P&L remain excluded | PNL-03 |
