...
| Market quotes and freshness | M2 - Implemented provider-independent quote/capture groundwork, Schwab adapter, bounded official equity evidence, temporary-DuckDB persistence contract, and interim one-app quote-evidence bridge | Not accepted | ADR-0011 and its acceptance matrix approve same-connected-broker provider-native schedule authority, with external and cross-provider fallback rejected. The current `v1` session object lacks provider/connection binding and no resolver is wired, so the bounded official quote remains unavailable for valuation. OneBot/VPS is only the temporary token owner; the versioned resolver, target connector plane, broader provider/asset evidence, approved runtime ingestion/migration, single-owner cutover, and end-to-end validation remain open | PNL-02 |
