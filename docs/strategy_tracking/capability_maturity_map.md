...
| Production website | M3 - Validated, with one operationally accepted local-owner slice | Not accepted as a complete production product | ADR-0017 defines the React/TypeScript/Vite/FastAPI foundation. WEB-W06 is owner-accepted for its private local loopback scope. WEB-W07's exact broker-current API/application path passes temporary-database and frontend validation, but migration 0022, accepted-result persistence, route activation, real loopback acceptance, authentication, hosted runtime, deployment, and complete production validation remain outstanding | P1-01 through P1-12, WEB-01 through WEB-09, and WEB-W01 through WEB-W13 |
