...
Snapshot date: 2026-08-28. This summary is derived from the current product
...
| Market quotes and freshness | M2 - Implemented provider-independent quote/capture groundwork, Schwab stock/option adapter, bounded official quote and schedule evidence, temporary-DuckDB persistence contract, provider-native session `v2`, and interim one-app quote-evidence bridge | Not accepted | ADR-0011 and its acceptance matrix approve same-connected-broker schedule authority, and the common resolver/freshness/importer boundary enforces exact provider/connection/quote/instrument/source lineage. The bounded T14 capture proves official real-time stock/option shapes and exact normal, closed-sentinel, and shortened-session payload compatibility. The approved offline resolver adds `closed_unspecified`, offset-validated Schwab product/timezone mappings, shortened-session classification, and combined-manifest lineage. The captured quote and schedule dates still differ, so actual v2 authority and valuation remain unavailable. OneBot/VPS is only the temporary token owner; same-date T14 evidence, target connector activation, approved runtime ingestion/migration, single-owner cutover, and end-to-end validation remain open | PNL-02 |
