...
| Market quotes and freshness | M2 - Implemented provider-independent quote/capture groundwork, Schwab adapter, bounded official equity evidence, temporary-DuckDB persistence contract, provider-native session `v2`, and interim one-app quote-evidence bridge | Not accepted | ADR-0011 and its acceptance matrix approve same-connected-broker schedule authority, and the common resolver/freshness/importer boundary now enforces exact provider/connection/quote/instrument/source lineage. No concrete Schwab schedule adapter or official schedule response is accepted, so the bounded quote remains unavailable for valuation. OneBot/VPS is only the temporary token owner; provider adapter evidence, the target connector plane, broader provider/asset evidence, approved runtime ingestion/migration, single-owner cutover, and end-to-end validation remain open | PNL-02 |
