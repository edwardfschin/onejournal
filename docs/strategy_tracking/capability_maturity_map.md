...
| Market quotes and freshness | M2 - Implemented provider-independent quote/capture groundwork, offline Schwab adapter, temporary-DuckDB persistence contract, and interim one-app quote-evidence bridge | Not accepted | OneBot/VPS is only the temporary single Schwab-token owner; the target OneJournal-owned connector plane must support Schwab, IBKR, Moomoo, and later providers behind common contracts. Approved provider capture, private transfer, entitlement verification, a sanitized official fixture, connector security/ownership, approved runtime ingestion/migration, single-owner cutover, and end-to-end validation remain open | PNL-02 |
