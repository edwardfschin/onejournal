...
| `scripts/web/run_local_owner_journal_api.py` | ACTIVE | Starts the v3 private journal API on `127.0.0.1` against one operator-selected, pre-existing non-symlink mode-0600 migration-0020 DuckDB with complete execution and lifecycle projections; it performs no migration or provider access. | WEB-W06 local-owner browser workflow only; hosted access and production authentication remain unapproved. |
