...
[legacy-code audit](legacy_code_audit.md). Last reviewed: 2026-09-06.
...
| `scripts/journal/calculate_phase1_realized_result.py` | ACTIVE | Read-only calculation of one deterministic, fingerprinted realized-P&L result from the exact active history revisions. Allocations touching unresolved scopes are withheld and represented by value-free omission counts. | WEB-W08 Gate 3 preparation only; no acceptance, persistence, provider, or order authority. |
| `scripts/journal/prepare_phase1_reporting_release.py` | ACTIVE | Rebuilds exact accepted WEB-W08 current and realized authorities, binds owner-chosen private aliases and separate acceptance lineage, persists twice only to a new disposable database copy, verifies exact read-back/source immutability, and writes a value-free mode-`0600` private package. | WEB-W08 Gate 4 preparation and rehearsal only; it never writes the source database, restarts the API, calls a provider, or accesses an order endpoint. |
| `scripts/journal/persist_phase1_reporting_release.py` | MAINTENANCE_EXPLICIT | Dry-runs by default by rebuilding the exact prepared WEB-W08 release and checking package authorization, database/release fingerprints, migration 0026, empty or identical state, and privacy-safe counts. `--persist` additionally requires a distinct byte-identical mode-`0600` backup before one transactional append and exact read-back. | WEB-W08 Gate 4 persistence boundary only; it cannot migrate, call a provider, access credentials or orders, activate authorization, or restart the API. |
...
| `scripts/journal/validate_external_schwab_lifecycle_acquisition.py` | ACTIVE | Reads one exact owner-private paired Schwab order/transaction bundle and account binding, converts in memory, and emits privacy-safe counts/digests only. | PNL-03L credential-free validation; no provider, credential, private-write, or database capability. |
| `scripts/journal/ingest_schwab_phase1_evidence_assembly.py` | ACTIVE | Validates one pre-existing mode-0600 Phase 1 Schwab assembly by default; `--persist` atomically writes all eight families to a pre-existing mode-0600 migration-0016 DuckDB and verifies exact read-back. | P1-05 credential-free import boundary; no provider, credential, migration, raw-evidence write, or order capability. |
| `scripts/journal/materialize_schwab_phase1_journal.py` | ACTIVE | Materializes one exact persisted Phase 1 assembly into canonical fills and conservative resolved/review-required journal episodes in a pre-existing mode-0600 migration-0018 DuckDB; identical replay verifies lineage without duplicate writes. | P1-06 private operational-journal gate; no provider, credential, database creation, migration, financial P&L, or order capability. |
| `scripts/journal/project_schwab_phase1_executions.py` | ACTIVE | Builds one exact migration-0019 execution-first projection from a named migration-0018 materialization, groups partial executions by canonical instrument, corrects fill-count strategy misclassification, and requires every calculated cash movement to match its exact Schwab transaction. | P1-06 root-cause correction; no provider, credential, migration, evidence rewrite, P&L, or order capability. |
| `scripts/journal/reconcile_schwab_phase1_journal_lifecycle.py` | ACTIVE | Persists one exact ten-family assembly v2 and its additive terminal-event/current-position lifecycle projection to a pre-existing mode-0600 migration-0020 journal; identical replay is verified and audit is value-free. | P1-06 lifecycle root-cause correction; no provider, credential, migration, evidence rewrite, P&L, or order capability. |
| `scripts/journal/revise_schwab_history_materialization.py` | MAINTENANCE_EXPLICIT | Persists one exact assembly v2, snapshots the existing materialization once, appends a complete revision, and selects it through an activation event in a pre-existing mode-0600 migration-0021 journal. | P1-06 history-extension gate; no provider, credential, database creation, migration, deletion, P&L, or order capability. |
| `scripts/web/run_local_owner_journal_api.py` | ACTIVE | Starts the v5 private journal API on `127.0.0.1` against one operator-selected, pre-existing non-symlink mode-0600 migrated DuckDB with complete execution and lifecycle projections. Optional exact private authorization files activate persisted broker-current and Phase 1 reporting releases; the launcher performs no calculation, persistence, migration, or provider access. | WEB-W06 through WEB-W08 local-owner browser workflows only; hosted access and production authentication remain unapproved. |
| `scripts/private/sync_onejournal_private_state.py` | MAINTENANCE_EXPLICIT | Plans or applies a one-way SSH/rsync private-workstation handoff. Immutable evidence is add-only and conflict-blocking; the operational DuckDB uses closed-handle checks, exact checksums, a staging file, destination backup, and atomic activation. | Owner-operated Mac-to-Mac private state only; no Git, provider, credential, deployment, delete, or conflict-resolution authority. |
