...
| PNL-02 | IN PROGRESS | Select a market-data provider and define quote ingestion, storage, licensing, and freshness. | ADR-0009 accepts Schwab first, IBKR next, and Moomoo later through a provider-independent, local-only quote contract. Repository-policy loading, migrations 0011/0012, a provider-neutral complete-capture envelope, atomic normalized storage, full-envelope replay lineage, and fail-closed tests are implemented. ADR-0011 approves same-connected-broker schedule evidence as the exclusive session authority, with no other-broker, third-party-calendar, or clock fallback. T09 implements the versioned provider-native authority value, exact same-provider/connection/quote/instrument/source binding, separate quote/evaluation phases, freshness integration, and a credential-free importer resolver seam. ADR-0012 accepts the isolated provider-connector and single-credential-ownership boundary. ADR-0013 and T11 enforce a versioned local-owner provider-use profile, exact connection-scoped acknowledgement, provider-entitlement non-substitution, and manual-only audited raw-evidence deletion authorization. T12/T13 implement the offline isolated connector plus restart-safe private envelope recovery, explicit guarded persistence, exact replay, scoped read-back, audit output, and real-journal migration/approval procedure using only synthetic evidence and temporary databases. T14 is complete for the bounded Schwab scope: checksum-bound capture `PNL-02-T14-SCHWAB-20260829-06` supplies same-date official real-time stock, listed-option, and normal-session evidence plus closed-sentinel and shortened schedules; `schwab-market-hours-resolver-v1` produces combined v2 authority; and `schwab-quote-json-v2` preserves the observed equity `securityStatus=Closed` as a frozen, session-unknown quote that is eligible only after exact authority confirms effective close. No actual journal migration/write or OneJournal provider activation is accepted. ADR-0016 accepts bounded bridge mode as the current PNL-02 completion route while OneBot/VPS remains the sole Schwab credential owner; the isolated OneJournal-owned connector and break-before-make cutover remain later target-architecture work. Completion now requires T16 bridge-mode end-to-end owner acceptance and T17 closure; T15 is non-blocking. |
...
| PNL-02-T16 | IN PROGRESS | Implement, run, and reconcile bounded end-to-end bridge-mode acceptance from exact externally acquired Schwab provider bytes and verified acquisition lineage through OneJournal's credential-free intake, adapter, capture envelope, session authority, guarded durable persistence, scoped read-back, and freshness decision, including negative fail-closed and secret/order safety cases. | The credential-free `onejournal.external-provider-acquisition.v1` intake and deterministic conversion slice is implemented with canonical owner/epoch/approval/acknowledgement/source/request/response/count/lifecycle validation, exact Schwab quote and market-hours allowlists, OneJournal-owned quote and schedule conversion, and checksum-derived capture identity. The guarded filesystem operator defaults to validation-only, requires exact owner-only bundle permissions and explicit quote/session mappings, resolves freshness before any write, optionally requires valuation eligibility, and permits append-only private materialization only under an explicit flag; it has no provider, credential, refresh, account, order, migration, database, scheduling, or listener capability. A synthetic end-to-end acceptance test proves acquisition validation, conversion, exact provider-native authority, freshness, private materialization, prepared temporary-DuckDB persistence, exact scoped read-back, and idempotent replay as one chain. Focused negative tests reject missing/altered or credential-bearing manifests, owner/epoch/approval/acknowledgement mismatch, incomplete acquisition, account/order activity, unsupported endpoint/method/query/status, stale, delayed, crossed, future, authority-absent session-unknown evidence, unsafe permissions, and overwrite. Remaining acceptance requires an explicitly approved bounded acquisition inside the sole OneBot owner boundary, checksum-preserving private transfer, separately approved real private materialization and local DuckDB migration/write/read-back, a dated evidence pack, and explicit project-owner acceptance. It does not depend on T15 and does not authorize credentials, scheduling, a public listener, website-triggered access, or OneBot-derived normalized state. |
