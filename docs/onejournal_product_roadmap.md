...
| PNL-02-T15 | BLOCKED | Execute the explicit single-owner Schwab cutover: retire OneBot's Schwab access before OneJournal becomes the only token-lifecycle owner, then prove there is no dual refresh or provider-call path. | The credential-free `onejournal.provider-connection-cutover.v1` boundary validates the exact `source_active -> owner_gap -> target_provisioned_disabled -> target_active` sequence for forward cutover and rollback, including distinct hosts, new owner/credential lineage, target isolation, ordered evidence hashes, and fail-closed overlap. On 2026-08-29 the owner approved the current Mac as the temporary local-only staging host, using macOS Keychain with no public listener, DuckDB mount, hosted storage, or OneBot token copy. The checked-in `onejournal.macos-provider-staging.v1` foundation remains fully disabled and uses only fake Keychain evidence in tests. `onejournal.macos-staging-rehearsal.v1` now emits only a secret-free stdout record after resolving its own clean local Git artifact and rejecting any enabled capability; the first valid local run must wait until that runner is committed. Current authenticated terms/acknowledgement confirmation, fresh credential handling after the owner gap, exact OneBot retirement, bounded OneJournal activation/provider access, operational evidence, and rollback remain explicit gates. Accepted ADR-0015 adds a temporary evidence-only operating mode that keeps OneBot as sole token owner while OneJournal accepts only exact provider bytes and verified acquisition lineage; it neither changes T15's status nor completes T15, T16, T17, or PNL-02 unless separately approved. |
