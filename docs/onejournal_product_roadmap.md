...
| PNL-02-T16 | IN PROGRESS | Implement, run, and reconcile bounded end-to-end bridge-mode acceptance from exact externally acquired Schwab provider bytes and verified acquisition lineage through OneJournal's credential-free intake, adapter, capture envelope, session authority, guarded durable persistence, scoped read-back, and freshness decision, including negative fail-closed and secret/order safety cases. | The credential-free `onejournal.external-provider-acquisition.v1` intake and deterministic in-memory conversion slice is implemented with canonical owner/epoch/approval/acknowledgement/source/request/response/count/lifecycle validation, exact Schwab quote and market-hours allowlists, OneJournal-owned quote and schedule conversion, and checksum-derived capture identity. A synthetic end-to-end acceptance test now proves acquisition validation, OneJournal-owned quote and schedule conversion, exact provider-native authority, freshness, append-only private materialization, prepared temporary-DuckDB persistence, exact scoped read-back, and idempotent replay as one chain. Focused negative tests reject missing/altered or credential-bearing manifests, owner/epoch/approval/acknowledgement mismatch, incomplete acquisition, account/order activity, unsupported endpoint/method/query/status, stale, delayed, crossed, future, and authority-absent session-unknown evidence. Remaining acceptance requires an explicitly approved bounded acquisition inside the sole OneBot owner boundary, checksum-preserving private transfer, separately approved private materialization and local DuckDB migration/write/read-back, a dated evidence pack, and explicit project-owner acceptance. It does not depend on T15 and does not authorize credentials, scheduling, a public listener, website-triggered access, or OneBot-derived normalized state. |
