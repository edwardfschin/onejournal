...
Phase 1 progress: **5 of 12 work packages complete**.
...
| P1-07 | IN PROGRESS | Approve and deliver canonical current positions, cost basis, marks, market value, and unrealized P&L. | PNL-03 is complete and owner-accepted for the bounded private broker-current scope. WEB-W07 now adds an exact-run loopback API and `/portfolio/local` application slice with explicit as-of, source/run/snapshot/fingerprint/acceptance lineage, currency quantum, independent per-metric availability, and complete-total gates. Temporary-DuckDB tests prove a 58-position result with 58/58 cost-basis, market-value, and reconciled-unrealized availability, exact authorization matching, privacy-safe audit, and unavailable behavior without authorization; all 514 clean-CI tests, the frontend build, and focused frontend lint pass with no synthetic or FIFO fallback. The operational journal remains at migration 0021 with zero broker-current rows. Migration 0022 application, exact accepted-result persistence, route activation, real loopback verification, and owner acceptance remain separately gated; no provider, credential, VPS, deployment, or trading action occurred. |
...
| PNL-03 | COMPLETE | Implement current positions, cost basis, market value, and unrealized P&L. | PNL-03A through X preserve the historical fail-closed FIFO route and introduce the separately labelled ADR-0023 broker-reconciled current-position view. Owner-private replacement package `PNL-03X-BROKER-CURRENT-20260905-02` uses direct checksum-bound Schwab v3 position evidence, retains the exact currency quantum, and independently makes cost basis, market value, and unrealized P&L available for all 58 current positions; its calculation reconciles every position to Schwab open P&L. Separate package `PNL-03X-BROKER-CURRENT-ACCEPTANCE-20260905-01` records explicit owner acceptance of that exact run and result fingerprint. Migration 0015, guarded persistence, and an unregistered private serializer are validated locally without changing FIFO or the historical `46/4/3` and `48/10` evidence. Completion is limited to the private single-owner, broker-current scope: no actual journal migration, authenticated route, production release, Commercial API parity, realized-P&L completion for transferred lots, commit, push, sync, or deployment is claimed. |
...
| PNL-03Y | COMPLETE | Persist, privately serialize, and accept the exact 58-position broker-current result. | Additive migration 0015 retains direction-specific broker aggregate basis and adds separate broker-current run, position, rounding-policy, availability, reconciliation, and complete-total tables. The guarded repository reconstructs the result from its snapshot before atomic append, accepts only identical replay, and reads one explicit run. The unregistered private response contract withholds values until a release authorization matches both run UID and result fingerprint. Replacement private result `PNL-03X-BROKER-CURRENT-20260905-02` preserves `-01`, includes explicit USD quantum lineage, and reports 58/58/58 metric availability; separate owner-acceptance package `PNL-03X-BROKER-CURRENT-ACCEPTANCE-20260905-01` accepts that exact broker-current result. Tests use temporary DuckDB only; no actual journal migration, route activation, authentication change, provider call, commit, push, sync, or deployment occurred. |
...
| WEB-07 | IN PROGRESS | Implement local-owner journal, portfolio, P&L, trade, report, data-health, and settings vertical slices. | WEB-W06 is owner-accepted for the current local release scope. WEB-W07 has implemented and temporarily validated the exact accepted-result API/application path, but its operational migration, accepted-result persistence, route activation, real loopback verification, and owner acceptance remain separately gated. Later report, data-health, settings, and broader P&L slices remain open. |
...
| WEB-W07 | IN PROGRESS | The additive loopback-only route and `/portfolio/local` slice consume one explicitly authorized repository run, preserve exact as-of and acceptance lineage, validate independent metric/total availability, and fail closed without the accepted run. Temporary-DuckDB tests prove 58/58/58 metric availability and privacy-safe auditing without using the historical 48/10 FIFO subtotal; all 514 clean-CI tests, the frontend build, and focused frontend lint pass. Operational migration 0022, accepted-result persistence, route activation, real loopback verification, and owner acceptance remain gated. |
