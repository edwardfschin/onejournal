...
| PNL-02-T12 | COMPLETE | Implement the isolated, read-only OneJournal Schwab connector behind the common capture contract, with exact immutable raw capture and no account/order/database responsibilities. | Offline-only `onejournal.schwab.quote-connector.v1` validates the T11 acknowledgement before a fixed quote operation, serializes an injected owner lease, generation-checks an opaque credential capability, atomically writes only synthetic private 0700/0600 response/manifest bundles without overwrite or delete capability, invokes the existing credential-free adapter and complete-capture contract, and emits a secret-free `captured_private_uningested` audit. `src/onejournal/provider_connectors/schwab_quotes.py`, `src/onejournal/provider_connectors/private_capture.py`, `docs/schwab_isolated_quote_connector_contract.md`, and focused tests prove no shipped network, credential backend, database, account, transaction, position, order, scheduler, CLI, or listener capability. Schwab remains disabled; no credential, provider call, real evidence, activation, or deployment is claimed. T13 is required before journal ingestion or database persistence. |
