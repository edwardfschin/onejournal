...
| P1-06 | IN PROGRESS | Deliver the private local-owner trade and journal vertical slice through the new application boundary. | WEB-W06 uses an isolated `0600` local-owner DuckDB and no Streamlit or provider call. The immutable P1-05 v1 assembly and migration-0018/0019 projections remain historical evidence: 430 Schwab executions, 217 instrument lifecycles, 430/430 exact cash reconciliation, and zero false single-instrument multi-leg labels. ADR-0025, ten-family assembly v2, and migration 0020 reconcile terminal event quantities FIFO and require remaining quantities to match the complete current-position snapshot: 136 closed, 49 open, and 32 review-required historical lifecycles; all 49 open rows match current Schwab positions. The approved operational migration has a checksum-matched `0600` pre-0020 backup, exact replay, clean database integrity, and real loopback API/UI verification. ADR-0026 groups 68 exact vertical opening orders into 63 presentation pairs while preserving their instrument lifecycles. ADR-0027 and API/UI v5 add source-derived opening/closing phase stories, four exactly matched option-to-stock assignment/exercise links including the MSFT covered-call stock disposition, and 14 exact Schwab-indicated expiration stories (12 short puts, one short call, and one long put). ADR-0028 and migration 0021 add an append-only history-revision and activation model; a real-evidence disposable rehearsal preserved the 217-episode/430-fill legacy revision, appended a 267-episode/539-fill reconstruction by exactly reusing 430 fills and adding 109, exposed no duplicate current fills, corrected ETR to one closed two-execution Buy Call lifecycle, and proved activation-only rollback to 217 and restoration to 267 episodes. The operational database remains unchanged pending separate approval. Expirations remain `review_required`, linked stock cards are presentation-de-duplicated without merging records, ambiguous relationships fail closed, provider identities remain private, and no journal state or P&L authority changes. Owner visual acceptance remains before `COMPLETE`. |
...
| WEB-W06 | IN PROGRESS | The operational migration-0020 local-owner journal and v5 API/UI preserve 136 closed, 49 position-matched open, and 32 review-required instrument lifecycles without changing P&L authority. ADR-0026 derives 63 unique vertical presentation pairs from 68 exact filled Schwab opening orders. ADR-0027 adds exact opening/closing phase stories, four uniquely matched Schwab option assignments to their resulting stock executions (including MSFT shares called away), and 14 exact Schwab-indicated expiration stories; all expiration outcomes remain `review_required`. ADR-0028 now provides the locally tested append-only revision gate required before broader history acquisition; migration 0021 and the 539-fill reconstruction have passed a real-data disposable rehearsal but are not applied to the operational journal. Linked stock cards are suppressed only as list duplicates, while exact stock lifecycles remain independently inspectable. The denser two-line views increase text size, omit Mark and P&L, and preserve exact inspection. Owner visual acceptance remains. |
