...
| PNL-02-T09 | NOT STARTED | Implement the approved provider-native resolver boundary and wire exact session-authority observations into the credential-free importer and common freshness/read boundary without persisting freshness as a permanent quote property. | Implement a new contract version rather than reinterpret `v1`; bind provider, connection, provider instrument, schedule scope, source lineage, and optional evidence-backed MIC. Validation must prove deterministic identity, exact same-provider/same-connection binding, market-date/timezone correctness, and fail-closed resolver absence, expiry, cross-provider substitution, mismatch, conflict, and outage behavior. No provider call or credential use is authorized by this task state. |
| PNL-02-T10 | NOT STARTED | Approve the target OneJournal provider-connector security and ownership design for the bounded local-owner scope and later provider adapters. Completion requires credential storage and refresh ownership, opaque connection identity, process/deployment isolation, least privilege, secret-safe logs, rate-limit/retry behavior, audit boundaries, no-order capability, recovery, rollback, and a no-dual-owner Schwab cutover plan. | Architecture/security decision requiring project-owner approval. It must not silently choose a production authentication or tenancy model. OneBot remains the temporary Schwab token owner until PNL-02-T15. |
| PNL-02-T11 | BLOCKED | Make provider terms, connection-scoped acknowledgement, entitlement verification, permitted local use, and raw-data retention/deletion behavior enforceable for the approved operating scope. | Depends on PNL-02-T10 and current official Schwab terms. Any production user/connection persistence or UI remains blocked on separately approved authentication and tenancy; acknowledgement cannot replace provider-reported entitlement. |
| PNL-02-T12 | BLOCKED | Implement the isolated, read-only OneJournal Schwab connector behind the common capture contract, with exact immutable raw capture and no account/order/database responsibilities. | Depends on PNL-02-T10 and T11. Credential installation/use, provider access, network validation, deployment, and token refresh are separate explicit approvals. Offline implementation and tests must precede any live access. |
...
| PNL-02-T15 | BLOCKED | Execute the explicit single-owner Schwab cutover: retire OneBot's Schwab access before OneJournal becomes the only token-lifecycle owner, then prove there is no dual refresh or provider-call path. | Depends on approved design and offline readiness from PNL-02-T10 through T14. Credential/token handling, OneBot retirement, OneJournal activation, deployment/synchronization, and rollback are explicit operational approval gates. |
...
- PNL-02: the project owner approved Schwab first, IBKR next, and Moomoo later,
  ...
  tenancy decisions. The owner approved provider-native schedule evidence from
  the same connected account broker as the exclusive session authority, with
  no external-calendar or cross-provider fallback. Completion remains gated on
  implementing and validating that versioned integration, an approved durable
