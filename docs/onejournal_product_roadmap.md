...
| PNL-02-T08 | NOT STARTED | Define the PNL-02 acceptance matrix and select the authoritative market-session/calendar resolver. Completion requires approved source, MIC/calendar/timezone coverage, regular/extended/closed/holiday/early-close/unscheduled-closure behavior, update/outage policy, licensing constraints, and exact real-evidence cases. | **Next task.** Requires project-owner approval because it selects a durable market-session source and fixes the remaining acceptance scope. No provider call, credential use, or runtime change is authorized by the design decision. |
| PNL-02-T09 | BLOCKED | Implement the approved resolver adapter and wire exact session-authority observations into the credential-free importer and common freshness/read boundary without persisting freshness as a permanent quote property. | Depends on PNL-02-T08. Validation must prove deterministic identity, source/version/validity lineage, exact instrument/evaluation binding, market-date/timezone correctness, and fail-closed resolver absence, expiry, mismatch, conflict, and outage behavior. |
