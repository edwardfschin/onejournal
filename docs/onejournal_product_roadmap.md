...
| PNL-03T | IN PROGRESS | Bind the frozen bounded scope to durable private identity evidence, eligible FIFO, and exact broker reconciliation. | The additive `onejournal.bounded-pnl03-fifo-reconciliation.v1` repository boundary now requires an approved private-binding digest, complete snapshot UID, exact lifecycle assembly/status counts, approved eligible-fill fingerprint, complete account/instrument membership, and transaction-authoritative normalized fills for only the eligible positions. It runs versioned FIFO, reconciles against the complete snapshot, preserves unresolved positions as unavailable, and can emit only an eligible cost-basis subtotal. Synthetic contract tests pass; this repository slice used no private evidence, so no durable real binding, eligible-46 replay, provider action, database action, operational acceptance, or financial acceptance has occurred. |
