...
| PNL-02-T15 | BLOCKED | Execute the explicit single-owner Schwab cutover: retire OneBot's Schwab access before OneJournal becomes the only token-lifecycle owner, then prove there is no dual refresh or provider-call path. | The credential-free `onejournal.provider-connection-cutover.v1` boundary now validates the exact `source_active -> owner_gap -> target_provisioned_disabled -> target_active` sequence for forward cutover and rollback, including distinct hosts, new owner/credential lineage, target isolation, ordered evidence hashes, and fail-closed overlap. This closes the offline rehearsal-contract gap only. ADR-0014 still requires an approved dedicated OneJournal staging host. The current provider-use profile forbids hosted storage. Host/security/storage approval, an updated authenticated terms profile, provider-disabled deployment rehearsal, credential handling, OneBot retirement, OneJournal activation, bounded provider access, operational evidence, and rollback remain explicit gates. |
