...
| PNL-02-T16 | NEXT | Implement, run, and reconcile bounded end-to-end bridge-mode acceptance from exact externally acquired Schwab provider bytes and verified acquisition lineage through OneJournal's credential-free intake, adapter, capture envelope, session authority, guarded durable persistence, scoped read-back, and freshness decision, including negative fail-closed and secret/order safety cases. | Requires `onejournal.external-provider-acquisition.v1`, an explicitly approved bounded acquisition inside the sole OneBot owner boundary, checksum-preserving private transfer, deterministic OneJournal conversion, a separately approved local DuckDB migration/write/read-back scope, a dated evidence pack, and explicit project-owner acceptance for the stated local bridge scope and limitations. It does not depend on T15 and does not authorize credentials, scheduling, a public listener, website-triggered access, or OneBot-derived normalized state. |
