...
| PNL-03 | IN PROGRESS | Implement current positions, cost basis, market value, and unrealized P&L. | The owner accepted ADR-0019 on 2026-08-31. PNL-03A through J establish typed instrument identity, complete-account position reconciliation, valuation-mark selection, temporary-DuckDB persistence, portfolio aggregation, the bounded position-evidence bridge, owner-private source capture, and real-shape adapter correction. PNL-03K transferred the 53-position bundle and proved that none of the seven identities in five accepted historical PNL-01 scopes overlaps a current position. ADR-0021 and PNL-03L add the paired bounded lifecycle-evidence intake; PNL-03M proves its first 30-day real Schwab window and corrects missing per-record currency from conflict-free explicit same-account/window provider evidence rather than a USD default. Current per-import `normalized_positions` remain prototype-only. Remaining gates are contiguous complete-history assembly, durable private account/instrument binding, resolution of transaction-only and lifecycle evidence, current-position/fill/lifecycle reconciliation, accepted valuation-mark evidence, versioned API/presentation wiring, production migration approval, and owner financial acceptance. |
