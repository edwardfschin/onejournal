...
| PNL-03 | IN PROGRESS | Implement current positions, cost basis, market value, and unrealized P&L. | The owner accepted ADR-0019 on 2026-08-31. PNL-03A provides typed `onejournal.instrument-identity.v1`; PNL-03B provides complete-account broker snapshot reconciliation with explicit age policy; PNL-03C provides quote-bound long-bid/short-ask and eligible closed-last selection; PNL-03D composes those boundaries around the cumulative FIFO/lifecycle engine, preserves broker-only discrepancies and full freshness/session lineage, calculates exact equity/option market value and unrealized P&L, and additively persists/reads/replays migration-0013 results in temporary DuckDB. PNL-03E adds a credential-free, checksum/account/scope-bound Schwab single-account position adapter with explicit canonical mappings and complete-empty-account semantics. PNL-03F adds pure all-or-nothing multi-leg strategy and same-currency portfolio aggregation with explicit counts and privacy-safe reason codes; no partial or false-zero total is emitted. PNL-03G through J establish the bounded position-evidence profile, guarded offline operator, owner-private source capture, and real-shape adapter correction. Current per-import `normalized_positions` remain prototype-only. Remaining gates are checksum-preserving transfer, explicit private identity binding, credential-free offline validation, complete fill/lifecycle reconciliation, accepted valuation-mark evidence, versioned API/presentation wiring, production migration approval, and owner financial acceptance. |
