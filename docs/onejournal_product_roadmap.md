...
| PNL-02-T14 | BLOCKED | Execute the approved broader conformance matrix across Schwab equity and listed-option shapes, required session states, entitlement/delay states, stale/future/crossed/missing evidence, exact identity, and provider-neutral non-fallback behavior. | Offline preflight on 2026-08-28 covers synthetic Schwab equity and listed-option mapping, regular/pre-market/after-hours/closed/holiday/early-close/unscheduled-closure/DST phases, delayed/denied/unknown entitlement, stale/future/crossed/missing-price evidence, exact provider/connection/quote/instrument/date/time/source binding, outage/expiry, atomic persistence/read-back/replay, and cross-provider fallback rejection. The owner accepted `docs/pnl02_t14_temporary_schwab_evidence_bridge.md` on 2026-08-28. Authenticated Schwab Market Data Production 1.0.0 review confirmed `GET /marketdata/v1/markets`, the `markets` equity/option array, future-only `date` range, provider product maps, `isOpen`, and offset-aware session intervals. The repository now implements the canonical private `onejournal.provider-usage-acknowledgement-artifact.v1` serializer/loader and append-only 0700/0600 store with a generated 128-bit connection UID; no real record was created. The corrected non-recurring, no-refresh v3 runner remains outside both repositories, validates and copies that full checksum-bound acknowledgement before token access, serializes repeated market query keys, validates exact schedule scope/date/product/session identity, binds the approved authentication/configuration paths to the modules actually imported, binds the approved Python path to the running interpreter, verifies the effective OS identity, and passed ten offline tests at SHA-256 `fc3ba1b4b313ec642ff7f0a2d5f7124e74eed12f086a264031f6bf3e707f2e06`; it has not been transferred or executed. Completion still requires explicit owner attestation and a separately approved private acknowledgement write, owner-approved source-contract bytes and capture metadata, a pre-existing 0700 VPS output root, a concrete Schwab provider-native schedule adapter, and bounded official equity, listed-option, normal-schedule, holiday, and early-close evidence. Provider calls, private evidence writes/transfers, commit, and push remain separate approvals. Live IBKR and Moomoo integrations remain later provider rollouts unless the owner explicitly expands PNL-02 acceptance scope. |
