...
| WEB-W06 | WEB-07 | IN PROGRESS | Local-owner journal vertical slice | Browse real historical trades and journal state through loopback API; create an approved append-only review without Streamlit | Migration 0020 is applied to the `0600` operational local journal with a checksum-matched rollback backup, exact replay, clean integrity, and real loopback verification. The 217 historical instrument lifecycles remain 136 closed, 49 position-matched open, and 32 review-required; 48 open rows are options and one is stock. ADR-0026 groups 68 exact filled vertical opening orders into 63 privacy-safe presentation pairs. ADR-0027 and API/UI v5 present exact opening/closing phases, link four uniquely matched Schwab option assignments to resulting stock executions including the MSFT covered-call stock disposition, and show 14 exact Schwab-indicated expiration stories while keeping every expiration `review_required`; ambiguous candidates remain unlinked. Linked stock cards are suppressed only as list duplicates and remain independently inspectable. The compact collapsible UI uses larger type and denser two-line rows for source-derived time, quantity, instruction, average fill, cash movement and fees. It omits Mark and P&L, preserves independent lifecycles and financial authority, and exposes no provider order/account/transaction identity. Owner visual acceptance remains | Sol High for financial lifecycle review; Terra Medium for later bounded UI work |
