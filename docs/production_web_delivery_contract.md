...
| WEB-W08 | Bounded PNL-06 through PNL-08 slices, WEB-07 | IN PROGRESS | Phase 1 account/symbol breakdown, date-filtered P&L history, export and quality-conformant views | Explore the accepted current portfolio and export the same bounded records and values | Gate 0 found one accepted current-valuation run but no operational realized-P&L rows or safe account label. Gate 1 was owner-approved on 2026-09-07: accepted ADR-0029 and `docs/phase1_reporting_release_contract.md` define the bounded report authority, private alias, exact reconciliation, API/CSV parity, quality states, and separate approval gates. Gate 2 implementation and all later runtime gates remain open | Sol High for financial contracts; Terra Medium for implementation |
