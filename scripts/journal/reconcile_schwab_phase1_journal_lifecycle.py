#!/usr/bin/env python3
"""Persist one assembly-v2 lifecycle projection into a private journal.

The command has no provider, credential, migration, database-creation, P&L, or
order capability. Both input files must already exist with mode 0600.
"""
...
from onejournal.journal.schwab_journal_lifecycle_reconciliation import (  # noqa: E402
    privacy_safe_lifecycle_reconciliation_audit,
)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        required=True,
        type=Path,
        help="absolute path to an immutable mode-0600 assembly-v2 artifact",
    )
    parser.add_argument(
        "--db",
        required=True,
        type=Path,
        help="absolute path to a pre-existing mode-0600 migration-0020 journal",
    )
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        plan = persist_schwab_journal_lifecycle_reconciliation(db_path, assembly)
        audit = privacy_safe_lifecycle_reconciliation_audit(plan)
        audit["evidence_operation"] = "persisted" if evidence.created else "replayed"
        print(json.dumps(audit, sort_keys=True, separators=(",", ":")))
        return 0
    except (duckdb.Error, OSError, RuntimeError, ValueError) as exc:
        print(
            f"Schwab Phase 1 journal lifecycle reconciliation failed: {exc}",
            file=sys.stderr,
        )
        return 1
